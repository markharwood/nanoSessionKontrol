...
#MH        self.song().add_record_mode_listener(self.__update_record_button_led)
        self.song().add_session_record_listener(self.__update_record_button_led)
        self.song().add_is_playing_listener(self.__update_play_button_led)
        self.song().add_metronome_listener(self.__update_loop_button_led)
#MH        self.song().add_loop_listener(self.__update_loop_button_led)
    ...
    def destroy(self):
#MH        self.song().remove_record_mode_listener(self.__update_record_button_led)
        self.song().remove_session_record_listener(self.__update_record_button_led)
        self.song().remove_is_playing_listener(self.__update_play_button_led)
#MH        self.song().remove_loop_listener(self.__update_loop_button_led)
        self.song().remove_metronome_listener(self.__update_loop_button_led)
...
#MH                self.__toggle_loop()
    ...
    def handle_transport_switch_ids(self, switch_id, value):
        if switch_id == SID_TRANSPORT_REWIND:
            if value == BUTTON_PRESSED:
                #MH - moved to shift key down selectsnum bars using rec button on channel 1-8
                #tracks = self.song().visible_tracks
                #newVal=self.getNumBarsFromArmedTrackNames()
                #if newVal==0:
                #    newVal=4
                #else:
                #    newVal+=1
                #    if newVal>8:
                #        newVal=1
                ## Label all record-armed tracks with the new bar length
                #for t in tracks:
                #    if t.arm:
                #        t.name=self.getTrackNameMinusBars(t.name)+" x"+str(newVal)
            ...
            if value == BUTTON_PRESSED:
                ...
                if bars==0:
                    bars=4
                beatsPerBar=int(self.song().signature_numerator)
                beats=bars*beatsPerBar
                self.song().trigger_session_record(beats)#MH
#MH                self.__toggle_record()

##MH  parses track names to see if we have a "foo x4" name
#    def getNumBarsFromArmedTrackNames(self):
#        firstTrack=None
#        existingVal=0
#        tracks = self.song().visible_tracks
#        for t in tracks:
#            if t.arm:
#                match=re.search(r' x(\d+)$', t.name)
#                if match is not None:
#                    existingVal = int(match.group(1))
#                    break
#        return existingVal

##MH  parses track names - given "foo x4" returns "foo"
#    def getTrackNameMinusBars(self, tname):
#        match=re.search(r'(.*) x(\d+)$', tname)
#        if match is not None:

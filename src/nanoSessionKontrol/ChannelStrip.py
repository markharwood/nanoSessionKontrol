            ...
            if sw_id - SID_RECORD_ARM_BASE is self.__strip_index:
                if value == BUTTON_PRESSED:
                    ...
                    if self.shift_is_pressed():
                        newVal=self.__strip_index
                        self.set_fixed_record_bar_length(newVal+1)
                    else:
...
#                    self.__toggle_arm_track(exclusive)
    ...
    def __toggle_mute_track(self):
        if self.__assigned_track:
            if self.shift_is_pressed():
                if self.__assigned_track.playing_slot_index >= 0:
                    self.__assigned_track.stop_all_clips()
                else:
                    sceneIndex= list(self.song().scenes).index(self.song().view.selected_scene)
                    clip=self.__assigned_track.clip_slots[sceneIndex].clip
                    if clip is not None:
                        clip.is_playing=True
            else:
                self.__assigned_track.mute = not self.__assigned_track.mute
#MH            self.__assigned_track.mute = not self.__assigned_track.mute
...
class MasterChannelStrip(MHControlComponent):
